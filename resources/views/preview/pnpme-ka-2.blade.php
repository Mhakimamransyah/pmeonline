@php
    ...
    $parameters = \App\Package::all()->first(function ($item) use ($cycle, $subject) {
        return $item->cycle_id == 2 && $item->subject_id == $subject->id;
    })->parameters;
    $f = new stdClass();
            ...
            <table class="table table-bordered">
                ...
                @foreach($parameters as $parameter)
                    <tr>
                        ...
                        <td>{{ $f->{str_replace(' ', '_', 'hasil_pengujian_'.$parameter->name)} or '' }}</td>
                        <td>{{ $f->{str_replace(' ', '_', 'ketidakpastian_'.$parameter->name)} or '' }}</td>
                        <td>{{ $f->{str_replace(' ', '_', 'metode_'.$parameter->name)} or '' }}</td>
                    </tr>
                @endforeach

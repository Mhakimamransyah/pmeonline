                    ...
                    @foreach($submits as $submit)
                        @php
                            ...
                            for($i = 0; $i < 10; $i++) {
                                    ...
                                    if($answer == ["Negatif"])
                                        ...
                                        foreach ($answer as $item) {
                                            try {
                                                ...
                                                array_push($answerPlasmodiums, $item);
                                                $answerStadium = $item;
                                                if ($answerStadium == "Skizon") {
                                                    $answerStadium = "Schizont";
                                                }
                                ...
                                // Score
                                $expected = isset($scoreValue->{'isi_benar_'.$i}) ? $scoreValue->{'isi_benar_'.$i} : array();
                                $expectedJumlahMalaria = isset($scoreValue->{'jumlah_malaria_seharusnya'}[$i]) ? $scoreValue->{'jumlah_malaria_seharusnya'}[$i] : 0;
                                ...
                                // dd($scoreValue);
                                    ...
                                    // $txt_expected = json_encode($scoreValue);
                                }
                                ...
                                // Counting Score
                                $currentScore = isset($scoreValue->{'nilai'}[$i]) ? $scoreValue->{'nilai'}[$i] : 0;

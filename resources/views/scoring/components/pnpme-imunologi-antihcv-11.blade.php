@php
    ...
    $packageInjectData = \App\Package::query()->where('name', 'pnpme-imunologi-antihcv-3')->get()->first();
...
<form id="submit-form" class="ui form" method="post" action="{{ route('installation.scoring.store', ['order_id' => request()->get('order_id')]) }}">
        ...
        @for($i = 0; $i < 3; $i++)
            <tr>
                ...
                <td class="center aligned">
                    {!! $f->{'kode_panel_'.$i} ?? '<i>Tidak diisi</i>' !!}
                </td>
                <td class="center aligned">
                    @php
    ...
    @for($h = 0; $h < 3; $h++)
        ...
        <h4 class="ui divider horizontal header">Panel {!! $f->{'kode_panel_'.$h} ?? '<i>Tidak diisi</i>' !!}</h4>
        ...
        <table class="ui table celled">
            ...
            <tr>
                <td colspan="4" class="right aligned"><b>{{ 'Penilaian Hasil Panel ' }} {!! $f->{'kode_panel_'.$h} ?? '<i>Tidak diisi</i>' !!} :&nbsp;&nbsp;</b></td>
                <td class="error">
    ...
    <table class="ui table celled">
        ...
        <tr>
            ...
            <th class="center aligned" style="width: 50%">Saran dari Peserta</th>
        </tr>
        </thead>
        <tbody>
        <tr>
